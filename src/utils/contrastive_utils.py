...
def train_epoch(model, loader, optimizer, temp):
    ...
    acc_groups = {
        g_idx: utils.AverageMeter() for g_idx in range(n_groups) if loader.dataset.group_counts[g_idx] > 0}
...
def eval(
    ...
    for epoch in range(finetune_epochs):
        ...
        acc_groups = {
            g_idx: utils.AverageMeter() for g_idx in range(n_groups) if finetune_loader.dataset.group_counts[g_idx] > 0}
