...
def train_epoch(model, loader, optimizer, criterion):
    ...
    acc_groups = {g_idx: AverageMeter() for g_idx in range(n_groups) if loader.dataset.group_counts[g_idx] > 0}
...
def eval(model, test_loader_dict):
    ...
    with torch.no_grad():
        ...
        for test_name, test_loader in test_loader_dict.items():
            acc_groups = {g_idx: AverageMeter() for g_idx in range(test_loader.dataset.n_groups) if test_loader.dataset.group_counts[g_idx] > 0}
