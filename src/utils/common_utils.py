...
def get_y_s(g, n_spurious_first_class):
    y = (g >= n_spurious_first_class)*1
    if isinstance(g, torch.Tensor) or isinstance(g, np.ndarray):
        s = g.copy()
        s[y==0] = g[y==0]
        s[y==1] = g[y==1] - n_spurious_first_class
    elif isinstance(g, int) or isinstance(g, float):
        if y==0: s = g
        else: s = g - n_spurious_first_class
    else:
        TypeError(f"Unsupported type {type(g)} for g.")
    return y, s
...
def get_results(acc_groups, get_ys_func, weighting=None):
    ...
    if weighting is not None:
        results.update({"weighted_mean_accuracy" : sum([acc_groups[g].avg * weighting[g] for g in groups])})
    return results
...
def get_data(args, logger=None, contrastive=False, finetune_on_val=False):
    ...
    loader_kwargs = {
        'batch_size': args.batch_size, 'num_workers': 12, 'pin_memory': True}
    sampler = data.get_sampler_training(trainset, args)
    ...
    get_ys_func = partial(get_y_s, n_spurious_first_class=testset_dict['test'].n_spurious)
